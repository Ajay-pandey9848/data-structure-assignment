...
class IntArray():
    ...
    def insert(self, index:int, val:int) -> None:
        """
        Insert a new element with value `val` at position `index`.
        Elements from `index` onwards are shifted to the right.
        """
        # Check value range
        if not isinstance(val, int) or not self._min_val <= val <= self._max_val:
            raise TypeError(f'Value must be an integer between {self._min_val} and {self._max_val}')

        # Check index validity (allow insert at the end: index == self._size)
        ...
        # Copy bytes before the insertion point (if any)
        if self._resmem and index > 0:
            bytes_before = index * self._bytes_per_element
            new_resmem.copy(self._resmem, count=bytes_before, source_index=0, destination_index=0)

        # Copy bytes after the insertion point (if any)
        if self._resmem and index < self._size:
            ...
            new_resmem.copy(self._resmem, count=bytes_after,
                            source_index=source_index,
                            destination_index=destination_index)

        # Replace reserved memory and update size
        self._resmem = new_resmem
        self._size = new_size

        # Store the new value at the insertion index
